...
public class AssetAndLiabilityAction extends TransactionAction {
		...
		try {
			if (request.getParameter(PARAM_ACTION) != null) {
				if (request.getParameter(PARAM_ACTION).equalsIgnoreCase(
						ACTION_SAVE_TT_LIST)) {
					transactionDetailHelper.saveAssetAndLiabilityDetails(
							transactionDetailForm.getAssetAndLiability(),
							session);
					actionMessages.add("transaction.save.success",
							new ActionMessage("transaction.save.success"));
				}
			}
			transactionDetailForm.setAssetAndLiabilityList(transactionDetailHelper
					.findAssetandLiability(session));
			request.setAttribute(ATTR_SEARCH_RESULT, transactionDetailForm.getAssetAndLiabilityList());
