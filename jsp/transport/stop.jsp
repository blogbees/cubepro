	...
	mmLoadMenus();	
	   function SubmitOnEnter(e) {
			...
			function submitSearch(){
				document.forms[0].action="<%=request.getContextPath()%>/Stop.cp";
				document.forms[0].submit();
			}
			
			function saveForm(){
				var termName = document.forms[0]['stopDetail.stopName'];
					if(IfNullorEmpty(termName.value)){
						alert("Enter stop name");
						termName.focus();
					}else{
						document.forms[0].action="<%=request.getContextPath()%>/Stop.cp?action=saveStop";
						document.forms[0].submit();
					}
			}
			...
			function resetFilters(){
				document.forms[0]['stopDetail.stopId'].value = 0;
				document.forms[0]['stopDetail.stopName'].value = "";
				document.forms[0]['stopDetail.stopDesc'].value = "";
			}
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td colspan="3" class="bread-crumb"><a href="<%=request.getContextPath()%>/Home.cp"><bean:message
					key="nrega.home" /></a> &gt; <span>Stop Master</span></td>
			</tr>
			...
			<tr>
				<td width="100%"><html:form action="Stop" method="post">
							...
							<table width="100%" border="0" cellspacing="1" cellpadding="0"
								...
								<tr>
									<td colspan="8" class="form-heading"><span
										class="search-heading">Stop Master </span></td>
								</tr>
								<tr>
									...
									<table width="100%" border="0" cellspacing="1" cellpadding="0"
										...
										<tr>
											<td class="form-row-text">Stop Name :</td>
											<td class="form-row-input"><html:text
												property="stopDetail.stopName" size="30"
												maxlength="45" styleId="searchProjectName"
												 />
												<html:hidden property="stopDetail.stopId" />
												</td>
											<td class="form-row-text">Stop Desc</td>
											<td class="form-row-input">
											<html:text
												property="stopDetail.stopDesc" size="45"
												maxlength="45" styleId="searchProjectName"
												 />
											</td>
										</tr>
								...
								<tr class="toggle-section0">
									<td><bean:define id="pageSizeList"
										name="stopForm" property="pageSizeList"
										toScope="page" /> <logic:empty property="stopList"
										name="stopForm">
										<bean:message key="no.search.projects" />
									</logic:empty> <logic:notEmpty property="stopList"
										name="stopForm">
										<tr>
											...
											<table width="100%" border="0" cellspacing="1"
												...
												<tr class="toggle-section1">
													<td colspan="8"><display:table id="stopId"
														name="searchResult" requestURI="Stop.cp"
														...
														<display:column title="Stop Id" property="stopId"
															style="width: 13%;" sortable="true"
															headerClass="grid-heading-row"></display:column>
														<display:column title="Stop Name"
															property="stopName" style="width: 13%" sortable="true"
															headerClass="grid-heading-row"></display:column>
														<display:column title="Stop Desc"
															property="stopDesc" style="width: 13%" sortable="true"
															headerClass="grid-heading-row"></display:column>	
														<display:column title=""
															href="Stop.cp?action=setStop&"
															paramId="stopId" paramProperty="stopId"
